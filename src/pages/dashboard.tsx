...
import { getCompanyDepartments } from 'apiSdk/company-departments';
import { CompanyDepartmentsInterface } from 'interfaces/company-departments';
import { getAppUpdates } from 'apiSdk/app-updates';
import { AppUpdatesInterface } from 'interfaces/app-updates';

import { AppSettingsListPage as ShowCaseList } from 'pages/app-settings';
...
function DashboardPage() {
  ...
  const companyDepartmentsStat = useSWR<PaginatedInterface<CompanyDepartmentsInterface>>(
    () => `/company-departments`,
    () => getCompanyDepartments({ limit: 1, offset: 0 }),
  );

  const appUpdatesStat = useSWR<PaginatedInterface<AppUpdatesInterface>>(
    () => `/app-updates`,
    () => getAppUpdates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: companyDepartmentsStat?.data?.totalCount,
      isLoading: companyDepartmentsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'App Updates',
      result: appUpdatesStat?.data?.totalCount,
      isLoading: appUpdatesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`app-settings`} passHref legacyBehavior>
                  <Button
